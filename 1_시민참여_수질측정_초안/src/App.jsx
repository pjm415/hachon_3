...
const INITIAL_RECORDS = [
  ...
  { id: 5, riverId: '2014A85', riverName: '괴정천', type: 'positive', tag: '수질 측정', text: 'DO 용존산소 9.2mg/L로 매우 우수한 1급수 상태 유지 중', author: '조성하 (봉사단)', time: '40분 전', badgeCount: 20, photo: 'https://images.unsplash.com/photo-1576086213369-97a306d36557?auto=format&fit=crop&w=600&q=80', likes: 35, comments: 4, lat: 35.1018, lng: 128.9622 }
];
...
export default function App() {
  ...
  const fetchRealtimeGps = () => {
    if (!navigator.geolocation) {
      setUserGpsLocation({
        ...
        address: `부산광역시 사하구 ${currentStation.river} 하단 지오빌1차 아파트 인근`
      });
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        setUserGpsLocation({
          lat: currentStation.lat || 35.1018,
          lng: currentStation.lng || 128.9622,
          address: `부산광역시 ${currentStation.river} 하단 지오빌1차 아파트 인근 (GPS 위치)`
        });
        setIsGpsLoading(false);
      },
  ...
  const handleUploadSubmit = (e) => {
    ...
    const newRecord = {
      ...
      likes: 1,
      comments: 0,
      lat: userGpsLocation ? userGpsLocation.lat : (currentStation.lat || 35.1018),
      lng: userGpsLocation ? userGpsLocation.lng : (currentStation.lng || 128.9622),
