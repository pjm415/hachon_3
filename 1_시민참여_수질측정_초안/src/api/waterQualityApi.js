...
export const BUSAN_RIVER_STATIONS = [
  ...
  { id: '2014A70', name: '동천 (범일교 지점)', river: '동천', defaultBod: 2.8, defaultDo: 7.1, defaultPh: 6.8, defaultTds: 210, temp: 19.1 },
...
export function getStationWaterData(stationId = '2014A65') {
  ...
  return {
    ...
    metrics: {
      ...
      grade: { value: evaluateGrade(station.defaultBod), status: station.defaultBod <= 2.0 ? "good" : station.defaultBod <= 3.0 ? "warning" : "danger", label: "통합 수질 지표" }
    },
...
export function getRiverSummaryText(stationId) {
  if (stationId === '2014A65') {
    return "지금 온천천은 용존산소가 풍부하고(9.2mg/L) BOD 수치가 낮아 물고기와 새들이 살기 아주 깨끗한 1급수 상태입니다.";
  } else if (stationId === '2014A70') {
    return "지금 동천은 수온(19.1°C) 상승과 BOD 수치(2.8ppm) 증가로 악취 주의가 필요한 2급수 보통 상태입니다.";
  } else {
    return "지금 괴정천은 총용존고형물(285ppm) 수치가 높아 유류 띠 관찰 및 수질 정화 조치가 필요한 3급수 위험 상태입니다.";
  }
}
...
function parseRealApiResponse(item) {
  ...
  return {
    ...
    summaryText: getRiverSummaryText(item.ITEM_CODE || '2014A65'),
